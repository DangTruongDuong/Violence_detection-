...
def run_camera_detection(model_path, camera_id=0, confidence_threshold=0.70, performance_mode="balanced", use_pose_mode=False, enable_person_detection=True):
    ...
    if not enable_person_detection:
        print("⭕ Detection Mode: NONE (No visualization)")
    elif use_pose_mode:
        print("🦴 Detection Mode: POSE SKELETON")
    else:
        print("📦 Detection Mode: BOUNDING BOX")
...
def run_video_detection(model_path, video_path, output_path=None, confidence_threshold=0.70, performance_mode="balanced", use_pose_mode=False, enable_person_detection=True):
...
class ViolenceDetectionGUI:
    ...
    def __init__(self):
        ...
        self.person_detection = tk.BooleanVar(value=True)
    ...
    def setup_ui(self):
        ...
        # Person detection
        tk.Checkbutton(params_card, text="👥 Enable person detection",
                      variable=self.person_detection, font=("Segoe UI", 11),
                      bg=self.card_bg, activebackground=self.card_bg).pack(anchor=tk.W, padx=20, pady=(0, 10))
    ...
    def start_detection(self):
        ...
        # Enable person detection only if mode is not "none"
        enable_person_detection = self.person_detection.get() and detection_mode != "none"
        ...
        if self.mode.get() == "camera":
            self.detection_thread = threading.Thread(
                target=lambda: run_camera_detection(
                    ...
                    use_pose_mode, enable_person_detection
                ), daemon=True
            )
        else:
            self.detection_thread = threading.Thread(
